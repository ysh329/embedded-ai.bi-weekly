...
> 导读：本次16条。【业界新闻】TensorFlow Lite亮相TF DevSummit性能方面支持OpenCL且CPU基于ruy性能也有提升等，微控制器方面持续发力；ARM Cortex-A78和旗舰X1的新架构发布，树莓派4发布；【论文】谷歌提出数据回波榨干训练时的GPU资源、CVPR2020不引入约束的剪枝方法，以及BERT压缩方面的DynaBERT。【开源】本月可能会有2家推理框架开源。华为Mindspore发布新版本支持二阶优化，训练速度快到爆炸、端上手写体训练项目MNIST-CoreML-Training；【博文】Tengine支持armv8.2的cpu fp16特性加速推理，且支持加载NCNN模型；一篇对TFLite的Delegate的科普；Face++在端侧框架cpu方面所做的工作总结；TVM在Web上对WebAssembly和WebGPU的尝试；探秘嵌入式系统内存管理机制。
