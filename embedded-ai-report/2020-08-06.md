...
- [
BNN模型的计算负载和内存占用都有较大优势，但实际性能在具体硬件的加速上一直表现不好，本文作者提出了一个在GPU上实现二元卷积网络推理的方法，重点是xnor卷积的优化。实验结果表明，在conv3x3的情况下，使用GPU可以有42.61倍的加速。  
代码：http://github.com/metcan/Binary-consolutional-Neural-Network-Inference-on-GPU
