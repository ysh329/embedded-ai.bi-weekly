...
摘要：优图实验室在9月11日发布TNN新版本——v0.2版本。TNN新版本在通用性、易用性、NPU支持上都有很大的提升。这次主要在通用性、易用性、NPU支持上有较多升级：
    1. 通用性：在ONNX作为中间格式转换的基础上，TNN新版本新增了模型转换工具，可直接将TF Lite的模型转为TNN的模型。同时在转换过程中，引入了两次模型优化：一次是针对模型本身的结构进行等效转换、层融合与常数折叠等优化；另一次是在TNN Runtime中需要输入输出大小才能进行的优化，如Reshape层相关的融合等；  
