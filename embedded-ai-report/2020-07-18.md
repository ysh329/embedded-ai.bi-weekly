...
- [2007.02269v2][Rethinking Bottleneck Structure for Efficient Mobile Network Design](https://arxiv.org/abs/2007.02269v2)  
摘要：本文对逆残差模块深入分析，发现可能带来信息丢失和梯度混淆。作者设计了称为沙漏块的新block，不仅能在高维上做特征映射和空间变换，还减少了信息丢失和梯度混淆的可能性。  
实验表明，新模块沙漏block效果拔群，不增加参数和计算量的前提下，ImageNet分类精度比MobileNetV2提高1.7%以上。Pascal Voc2007目标检测方面也有0.9%的mAP改进。再结合神经结构搜索方法DARTS的搜索空间，进一步验证了沙漏块的有效性。  
